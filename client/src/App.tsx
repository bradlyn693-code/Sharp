...
function PlanCard({ plan }: { plan: (typeof plans)[number] }) {
  ...
  return <article className={`plan-card ${plan.popular ? "plan-card-popular" : ""} ${"isAdmin" in plan && plan.isAdmin ? "plan-card-admin" : ""}`}>
    ...
    <div className="mt-5 flex items-end gap-1"><span className="text-lg font-extrabold leading-none text-[#d8b4fe]">{plan.currency}</span><span className="font-display text-[28px] font-semibold tracking-[-0.045em] text-white">{plan.price}</span><span className="mb-1 text-[11px] text-[#776a8f]">/ monthly</span></div>
